...
class _PaymentScreenState extends State<PaymentScreen> {
  ...
  Future<void> _fetchPaymentSettings() async {
    ...
    // 1. First, set initial state from cache
    if (mounted) {
      setState(() {
        _activeGateway = repo.activeGateway;
        _isUpiEnabled = repo.isUpiEnabled;
        _isGooglePlayEnabled = true; // Always show Play Store as alternative
      });
    }

    // 2. Force fresh fetch from server for critical payment screens
    try {
      await repo.fetchPaymentConfigs(forceRefresh: true);
      if (mounted) {
        setState(() {
          _activeGateway = repo.activeGateway;
          _isUpiEnabled = repo.isUpiEnabled;
          _isGooglePlayEnabled = true;
        });
      }
    } catch (e) {
      debugPrint('Error force refreshing payment settings: $e');
    }
  }
    ...
    try {
      ...
      if (orderData['success'] == true) {
        ...
        await handler.initiatePayment({
          ...orderData,
          'phone': phone,
          if (widget.googlePlayId != null &&
              widget.googlePlayId!.trim().isNotEmpty)
            'googlePlayId': widget.googlePlayId,
          if (widget.googlePlaySubId != null &&
              widget.googlePlaySubId!.trim().isNotEmpty)
            'googlePlaySubId': widget.googlePlaySubId
      ...
      if (verifyRes['success'] == true) {
        ...
        await AnalyticsService.logPurchase(
          ...
          widget.rzpPlanId ??
              ...
              'premium_plan',
          eventId:
              'purchase_${successData['paymentId'] ?? successData['orderId'] ?? _currentOrderId ?? widget.rzpPlanId ?? widget.googlePlayId ?? widget.offerId ?? 'premium_plan'}',
        );
        _confettiController.play();
        _showSuccessDialog();
      } else {
          ...
          AlertDialog(
            ...
            content: Column(
              ...
              children: [
                ...
                const Text(
                    "Your GoGo Premium subscription has been successfully initiated.",
  ...
  Widget build(BuildContext context) {
    String validityDate = DateFormat('dd MMM yyyy')
          ...
          Container(
            ...
            child: Row(
              ...
              children: [
                ...
                const Text("100% SECURE GOLD ACTIVATION",
                    style: TextStyle(
                        color: Colors.green,
                        fontWeight: FontWeight.bold,
                        letterSpacing: 0.8,
                    ...
                    Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          InkWell(
                            onTap: () =>
                                _startSubscription(), // Use default server gateway
                            borderRadius: BorderRadius.circular(12),
                            child: Container(
                            ...
                            child: Container(
                              ...
                              child: Row(
                                children: [
                                  ...
                                  const Spacer(),
                                  if (widget.googlePlayId != null &&
                                      widget.googlePlayId!.isNotEmpty)
                                    Padding(
