...
redisClient.on('ready', async () => {
    ...
    try {
        ...
        const User = require('./src/features/users/models/User');
...
// Models
const User = require('./src/features/users/models/User');
const Message = require('./src/features/chat/models/Message');
const Block = require('./src/features/chat/models/Block');

// Services
const analyticsService = require('./src/shared/services/analyticsService');
const revenueService = require('./src/features/payment/services/revenueService');
const notificationService = require('./src/shared/services/notificationService');
const randomMatchController = require('./src/features/randomMatch/controllers/RandomMatchController');
const aiPersonaService = require('./src/features/chat/ai/services/aiPersonaService');
const { normalize, phoneQuery } = require('./src/shared/utils/phoneUtils');
const { updateConversationSummary, resetUnreadCount, queueStatusUpdate } = require('./src/features/chat/utils/chatUtils');
...
// Cross-instance broadcast support: harmless with a single instance today,
// and lets the AI creator-reply worker's io.to(...).emit(...) calls (and any
// future extra gateway instances) reach the right socket without further changes.
io.adapter(createAdapter(pubClient, subClient));

// BullMQ (Redis-backed) queue for AI creator-reply generation. Kept off the
// main `redis` v4 client since BullMQ requires ioredis; separate connection
// so a slow/overloaded AI worker tier can never starve presence/chat Redis ops.
const bullConnection = new IORedis({
    host: process.env.REDIS_HOST || '127.0.0.1',
    port: parseInt(process.env.REDIS_PORT || '6379', 10),
    maxRetriesPerRequest: null
});
const aiReplyQueue = new Queue('ai-creator-replies', { connection: bullConnection });
...
// Runs in-process for now (single VPS). If split into a standalone process
// later, swap the `io` argument for a `@socket.io/redis-emitter` instance.
require('./src/features/chat/ai/workers/aiReplyWorker').start(io, bullConnection);
require('./src/features/chat/ai/workers/proactiveMessageWorker').start(io, bullConnection);
...
const UserController = require('./src/features/users/controllers/UserController');
...
app.get('/api/media/:filename', require('./src/features/chat/controllers/ChatController').serveSecureMedia);
...
const { isUser, isAdmin } = require('./src/shared/middleware/auth');

app.use('/api/user', require('./src/features/users/routes/userRoutes'));
app.use('/api/chat', require('./src/features/chat/routes/chatRoutes'));
app.use('/api/admin', require('./src/features/admin/routes/adminRoutes'));
app.use('/api/payment', require('./src/features/payment/routes/paymentRoutes'));
app.use('/api/review', require('./src/features/review/routes/reviewRoutes'));
...
io.on('connection', (socket) => {
    ...
    socket.on('send_message', async (data, callback) => {
        try {
            ...
            // --- AI CREATOR AUTO-REPLY TRIGGER ---
            // Fire-and-forget: never let AI enqueueing slow down or fail a real message send.
            if (data.type === 'text' && data.message) {
                aiPersonaService.isFeatureEnabled().then(async (enabled) => {
                    if (!enabled) return;
                    const receiverIsCreator = await User.exists({ phone: receiver, isCreator: true });
                    if (!receiverIsCreator) return;
                    await aiReplyQueue.add('generate-reply',
                        { roomId, creatorPhone: receiver, userPhone: myPhone, messageId: tempId.toString() },
                        { jobId: tempId.toString(), attempts: 2, backoff: { type: 'exponential', delay: 2000 }, removeOnComplete: 1000, removeOnFail: 1000 }
                    );
                }).catch((aiErr) => console.error('AI_REPLY_ENQUEUE_ERROR:', aiErr.message));
            }
