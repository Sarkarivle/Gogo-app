...
const Subscription = require('../../payment/models/Subscription');
const VerificationRequest = require('../../users/models/VerificationRequest');
...
const Campaign = require('../../marketing/models/Campaign');
const RecentPhoto = require('../../chat/models/RecentPhoto');
const Conversation = require('../../chat/models/Conversation');
const ConversationMetadata = require('../../chat/models/ConversationMetadata');
const analyticsService = require('../../../shared/services/analyticsService');
const revenueService = require('../../payment/services/revenueService');
...
const { compressImageFile } = require('../../../shared/utils/imageCompressor');
...
exports.getAllUsers = async (req, res) => {
    try {
        ...
        // Exclude admin-managed Creator profiles — this list is for real signed-up app users only.
        let q = { isCreator: { $ne: true } };
        ...
        if (!stats) {
            ...
            const [
                ...
                User.countDocuments({ isCreator: { $ne: true } }),
                User.countDocuments({
                    isOnline: true,
                    isCreator: { $ne: true },
                    $or: [{ hasCompletedOnboarding: true }, { dobYear: { $exists: true, $ne: null } }]
                }),
                User.countDocuments({ createdAt: { $gte: todayStart }, isCreator: { $ne: true } }),
                User.countDocuments({ isPremium: true, isCreator: { $ne: true } }),
                User.countDocuments({ isPremium: true, createdAt: { $gte: todayStart }, isCreator: { $ne: true } }),
                User.countDocuments({ hasCompletedOnboarding: false, dobYear: { $exists: false }, isCreator: { $ne: true } }),
                User.countDocuments({ 'subscription.status': 'active', 'subscription.autoRenew': true, isCreator: { $ne: true } }),
                User.countDocuments({ 'subscription.status': 'cancelled', isCreator: { $ne: true } }),
                User.countDocuments({ 'subscription.status': 'expired', isCreator: { $ne: true } }),
                User.countDocuments({ monetizationMode: 'adDriven', isCreator: { $ne: true } })
            ]);
...
async function performFullUserWipe(phone) {
    ...
    const PaymentTransaction = require('../../payment/models/PaymentTransaction');
    const RandomQueue = require('../../randomMatch/models/RandomQueue');
    const RandomRoom = require('../../randomMatch/models/RandomRoom');
...
// ============ CREATOR MANAGEMENT (Admin-seeded feed profiles) ============
exports.createCreator = async (req, res) => {
    try {
        const { name, age, about, isOnline } = req.body;
        ...
        // Compress the uploaded photo in place (smaller upload -> faster loading in-app),
        // without a visible quality drop.
        const compressedFilename = await compressImageFile(path.join('./uploads', req.file.filename));
        const photoUrl = `/api/media/${compressedFilename}`;
        // Synthetic unique 10-digit identity so this record behaves like a normal User (chat, presence, etc.)
        const fakePhone = '9' + Date.now().toString().slice(-9);

        // Note: name/age/bio are also all the AI reply pipeline needs (see
        // aiPersonaService.buildSystemPrompt) — no separate persona setup step.
        ...
        await logAction(req, 'CREATE_CREATOR', creator.phone, `Created creator profile: ${name}`);
        res.json({ success: true, creator });
    } catch (e) {
        console.error("CreateCreator Error:", e);
        res.status(500).json({ success: false, message: e.message });
    }
};
...
exports.getCreators = async (req, res) => {
    try {
        const creators = await User.find({ isCreator: true }).sort({ createdAt: -1 }).lean();
        res.json({ success: true, creators });
    } catch (e) {
        console.error("GetCreators Error:", e);
        res.status(500).json({ success: false });
    }
};
...
exports.deleteCreator = async (req, res) => {
    try {
        const creator = await User.findOneAndDelete({ _id: req.params.id, isCreator: true });
        if (!creator) return res.status(404).json({ success: false, message: "Creator not found" });
        await logAction(req, 'DELETE_CREATOR', creator.phone, `Deleted creator profile: ${creator.name}`);
        res.json({ success: true });
    } catch (e) {
        console.error("DeleteCreator Error:", e);
        res.status(500).json({ success: false });
    }
};
