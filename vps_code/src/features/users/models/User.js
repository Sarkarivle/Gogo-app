...
const UserSchema = new mongoose.Schema({
    ...
    // Separate currency from premium: spent per voice-call attempt to a
    // creator. Starts at 1 so every user's first call gets the full
    // ring/connect/greeting experience before they ever need to buy more.
    callCredits: { type: Number, default: 1 },
    ...
    // Admin-managed feed creator (fake identity shown in Discover feed instead of real users)
    isCreator: { type: Boolean, default: false, index: true },

    // Auto-generated once from the creator's bio (never admin-edited) so the AI
    // gives consistent answers about "herself" across conversations/time instead
    // of inventing a different favorite food each time she's asked.
    aiSelfFacts: [String],
    aiSelfFactsBioHash: String,
